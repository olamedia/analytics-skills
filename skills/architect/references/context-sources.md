...
Known project documentation files that the architect skill can read for pre-existing context. Check for these before scanning the codebase from scratch — if they exist and are up to date, use them to accelerate research.
...
1. At the start of research (whenever it's triggered), check if these files exist
...
4. If files are missing or stale, proceed with normal codebase scanning — but note that running `analyze-project` first would save time on future runs
5. These files provide **project-wide** context; the architect skill's `context-map.md` is **feature-specific** and focuses only on what's relevant to the current feature
